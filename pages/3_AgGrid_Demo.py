...
if enable_selection:
    ...
    if use_checkbox:
        groupSelectsChildren = st.sidebar.checkbox(
            "Group checkbox select children", value=True
        )
        groupSelectsFiltered = st.sidebar.checkbox(
            "Group checkbox includes filtered", value=True
        )

    if (selection_mode == "multiple") & (not use_checkbox):
        rowMultiSelectWithClick = st.sidebar.checkbox(
            "Multiselect with click (instead of holding CTRL)", value=False
        )
        if not rowMultiSelectWithClick:
            suppressRowDeselection = st.sidebar.checkbox(
                "Suppress deselection (while holding CTRL)", value=False
            )
        else:
...
if enable_pagination:
    ...
    if not paginationAutoSize:
        paginationPageSize = st.sidebar.number_input(
            "Page size", value=5, min_value=0, max_value=sample_size
        )
    st.sidebar.text("___")
...
if enable_pagination:
    if paginationAutoSize:
        ...
        gb.configure_pagination(
            paginationAutoPageSize=False, paginationPageSize=paginationPageSize
        )
...
with st.spinner("Displaying results..."):
    ...
    if not selected_df.empty:
        selected_data = selected_df.loc[:, ["apple", "banana", "chocolate"]].assign(
            source="selection"
        )
